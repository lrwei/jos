...
trap_init_percpu(void)
{
	...
	ts->ts_esp0 = KSTACKTOP - (KSTKSIZE + KSTKGAP) * cpunum();
	ts->ts_ss0 = GD_KD;
	...
	lidt(&idt_pd);
        ...
        wrmsr(IA32_SYSENTER_ESP, KSTACKTOP - (KSTKSIZE + KSTKGAP) * cpunum());
...
page_fault_handler(struct Trapframe *tf)
{
	...
	// LAB 4: Your code here.
        if (curenv->env_pgfault_upcall) {
            struct UTrapframe *utf = (void *) (tf->tf_esp <= USTACKTOP ?
